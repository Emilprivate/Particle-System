...
void AddObject(float x, float y, int s = 0) {
	...
	o.ax = 0; o.ay = 0;
	o.ox = 0; o.oy = 0;
	o.diameter = ((float)rand() / (float)RAND_MAX) * Particleoptions::size;
	o.mass = (o.diameter / 2) * Particleoptions::mass;
	o.friction = Particleoptions::friction;
	o.svel = Particleoptions::startvelocity;
	o.score = s;
...
void Particle::Update() {
	...
	if (Particleoptions::addObj) {
		for (int i = 0; i < Particleoptions::amount; i++) {
			float x = fabs(RandomFloat(minBoxWidth + (Particleoptions::size / 2), maxBoxWidth - (Particleoptions::size / 2)));
			float y = fabs(RandomFloat(minBoxHeight + (Particleoptions::size / 2), maxBoxHeight - (Particleoptions::size / 2)));
			AddObject(x, y);
		}
		Particleoptions::addObj = false;
	}

	if (Particleoptions::removeObj) {
		int removeAmount;

		if ((size_t)Particleoptions::amount > vecObjects.size()) {
			removeAmount = vecObjects.size();
		}
		else {
			removeAmount = Particleoptions::amount;
		}

		vecObjects.erase(vecObjects.begin(), vecObjects.begin() + removeAmount);
		Particleoptions::removeObj = false;
	}
